...
def save_feedback(feedback_data, blob_service_client):
    ...
    try:
        ...
        # If file doesn't exist, create a new DataFrame with the necessary columns
        existing_feedback_df = pd.DataFrame(columns=fieldnames)
        logger.info("Feedback file not found. Creating a new feedback file.")
    except Exception as e:
        # Log any unexpected exceptions
        logger.error(f"Unexpected error occurred while accessing feedback file: {e}")
        return
    ...
    # Save updated feedback to Azure Blob Storage
    try:
        output = io.StringIO()
        ...
        logger.error(f"Failed to save feedback to Azure Blob Storage: {e}")
...
def search_across_indexes(vector_stores: List[FAISS], query: str):
    ...
    for store in vector_stores:
        results = store.similarity_search(query, fetch_k=10, k=3)
